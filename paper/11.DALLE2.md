...
由diffusion models beat GANs on iamge synthesis提出；
...
缺点：输出两个是非常昂贵的，而且扩散模型都需要重新训练，所以训练成本增加不少；但真的挺好用，所以GLIDE，DALLE2，Imagen都用到了。
...
直接搞一个大模型，学一些从caption到image的融合特征，学就完了，这不行吗？作者在摘要中说，如果中间有一个显式的生成图像特征的过程，效果会好很多。
